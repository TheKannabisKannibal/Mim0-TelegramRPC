...
- ⏱️ Прогресс воспроизведения в реальном времени, включая корректное обновление после перемотки
- ⏸️ Отображение паузы
...
Mim0 намеренно является **Windows-first приложением**. Windows Media Session — основной источник метаданных воспроизведения в Windows, поэтому отдельного слоя абстракции под Linux/macOS сейчас нет.

Основные обязанности разделены следующим образом:

```text
Telegram / совместимый плеер
          ↓
Windows Media Session
          ↓
Program.cs ───────→ Discord IPC
     │
     └────────────→ CoverService → временный URL Litterbox
```

`Program.cs` координирует выбор медиасессии и состояние Rich Presence, а `CoverService` отвечает за проверку миниатюры, временную загрузку и короткоживущий кэш обложек в памяти.

Это позволяет сохранить проект простым и не добавлять интерфейсы/DI-инфраструктуру, которая пока принесла бы больше сложности, чем пользы.
...
Windows Media Session может предоставлять миниатюру текущего трека. Discord Rich Presence не может использовать произвольный локальный файл как внешнее изображение, поэтому при включённой обложке Mim0 временно загружает миниатюру в Litterbox.

- Загружается только обложка текущего трека.
- Для файла запрашивается срок хранения один час.
- Mim0 хранит небольшой кэш в памяти до 50 минут, поэтому возврат к недавно проигранному треку не вызывает повторную загрузку той же обложки.
- Кэш никогда не записывается на диск.
- Сообщения Telegram, чаты, контакты и аудио не загружаются.
- Если сервис загрузки недоступен, RPC продолжает работать и использует встроенный Discord asset `default`.
...
Версия проекта теперь централизована в `Directory.Build.props`, поэтому EXE, окно «О программе» и обычные локальные сборки используют одну и ту же версию. В GitHub Actions релизная версия по-прежнему берётся из Git tag.
...
- [x] Единый источник версии
- [x] Кэш временных загрузок обложек
- [x] Корректное обновление таймлайна после перемотки
- [x] Локализация трея без reflection
- [x] Расширенное определение Telegram source ID
- [ ] Более подробная диагностика ошибок Windows Media Session
- [ ] Улучшение локализации установщика

### В будущем

- [ ] Дополнительные пресеты оформления Rich Presence
- [ ] Более надёжное определение источника медиасессии
- [ ] Дальнейшее разделение media/Discord-логики по мере роста проекта

Поддержка Linux/macOS намеренно **не входит в текущий roadmap** — продукт построен вокруг Windows Media Session.
...
### Обложка не появляется

Сам Rich Presence не зависит от загрузки обложки. Если Litterbox недоступен или Windows Media Session предоставляет неподдерживаемый формат миниатюры, Mim0 продолжает показывать RPC и использует встроенный Discord fallback asset.
